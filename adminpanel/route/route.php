...
public static $static_url = "/asg/adminpanel/";
...
public static function requestPost($url , $data){
	...
	switch ($url) {
		case $url_prefix.'create_user':
			...
			//array_push($data,MoldUser::createUser($data) );
		...
		case $url_prefix.'add_team':

			

			//array_push($data,MoldUser::createUser($data) );
			$data = array_merge($data,MoldUser::addTeam($data));
		...
		case $url_prefix.'update_user':
			...
			$data = array_merge($data,MoldUser::updateUser($data));
...
public static function requestGet($url , $data){
	...
	switch ($url) {
		...
		if(array_key_exists("action",$data) && $data['action'] == 'search'){
			...
			$user_info = $data['values'];
			$data['values'] = array('sponser'=>$user_info['id'], 'name'=>$user_info['name']);
			$data['keys'] = MoldUserKey::getDataset($identy , 1)['data'];
		...
		Render::renderView($data , 'add_team.html');
