...
class User extends DbInit{
	...
	function login($data){
		...
		if($this->isConnected()){
			...
			$query = "SELECT pass FROM `users` WHERE id = '".$data['username']."' LIMIT 1";
			...
			if($row_data  = mysqli_fetch_assoc($result)){
				...
				if($password == $data['password']){
					...
					$data['status_detail'] = array("title"=>"success","description"=>"Password Matched");
					$data['login_status'] = true;

				}else{

					$data['status_detail'] = array("title"=>"failed","description"=>"Password Mismatch");
					$data['login_status'] = false;

				}
			}else{

				$data['status_detail'] = array("title"=>"failed","description"=>"User Not Found");
					$data['login_status'] = false;
				...
				$data['status_detail'] = array("title"=>"Error","description"=>mysqli_error($this->getConnection()));
				$data['status'] = false;
									$data['login_status'] = false;
				...
				$data['con_error'] = "true";
				$data['status_detail'] = array("title"=>"Error","description"=>"Cannot make connection to database");
				$data['status'] = false;
									$data['login_status'] = false;
	...
	function insert($data){
		if($this->isConnected()){
			...
			$query = "INSERT INTO `users`(`name`, `mobile`, `email`, `pass`, `code`, `address`, `add_key`, `status`, `created`, `updated`, `nominee`, `relation`, `dob`, `father_name`) VALUES ('".$data['name']."','".$data['mobile']."','".$data['email']."','".$data['pass']."','".$data['code']."','".$data['address']."','".$data['add_key']."','1',NOW(),NOW() ,'".$data['nominee']."' , '".$data['relation']."' , '".$data['dob']."','".$data['father_name']."' )";
	...
	function updateUser($data){
		...
		if($this->isConnected()){
			...
			$query = "UPDATE `users` SET `pass`='".$data['pass']."',`updated`= NOW() WHERE code = '".$data['code']."'";
